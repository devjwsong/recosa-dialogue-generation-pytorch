...
class CustomDataset(Dataset):
    def __init__(self, data_type, config):
        ...
        with open(f"{config['data_dir']}/{data_name}_id.txt", 'r') as f:
            lines = f.readlines()
            ...
            if line.strip() == config['dialogue_split_line']:
                ...
                if num_time < config['max_time']:
                    ...
                    if num_time < config['max_time']:
                        ...
                        e_mask = self.make_encoder_mask(num_time, config['max_time'])
                    else:
    ...
    def make_encoder_mask(self, num_time, max_time):
        e_mask = [1 for i in range(num_time+1)] + [0 for i in range(max_time-num_time-1)]  # (T)
